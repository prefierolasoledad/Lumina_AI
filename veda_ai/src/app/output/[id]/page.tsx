    ...
    <div className="min-h-screen bg-zinc-100 flex text-zinc-900 font-sans selection:bg-[#ff7a59] selection:text-white print:bg-white print:text-black">
      ...
      <div className="flex-1 flex flex-col min-w-0 print:block">
        ...
        <div className="hidden md:block no-print">
          <Header title="Create New" showBackButton={true} onBackClick={() => router.push('/')} />
        </div>
        ...
        <main className="flex-1 p-4 md:p-8 overflow-y-auto bg-[#333333] print:bg-white print:p-0 relative">
          ...
          <div className="max-w-4xl w-full mx-auto space-y-6 print:block print:space-y-0">
            ...
            {/* Header controls (Edit Mode & Regenerate) */}
            <div className="flex justify-between items-center no-print">
              <Link href="/" className="inline-flex items-center text-xs text-zinc-400 hover:text-[#ff7a59] transition-all font-bold gap-1 group hover:-translate-x-0.5">
                <span>←</span>
                <span>Back to Dashboard</span>
              </Link>
            </div>

            {/* Black Header Banner matching Image 2 */}
            <div className="w-full bg-[#1b1b1c] border border-zinc-800 rounded-3xl p-6 flex flex-col items-start gap-4 shadow-xl no-print text-white">
              <p className="text-sm font-semibold leading-relaxed w-full">
                {greetingText}
              </p>
              {/* Desktop Pill Button */}
              <button
                onClick={handlePrint}
                className="hidden md:flex px-6 py-2.5 bg-white hover:bg-zinc-100 text-zinc-900 font-extrabold rounded-full text-xs transition-all items-center justify-center gap-2 cursor-pointer shadow-md shrink-0 hover:scale-102 active:scale-98"
              >
              ...
              <button
                onClick={handlePrint}
                className="flex md:hidden w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 text-white items-center justify-center transition-all cursor-pointer shadow-md hover:scale-105 active:scale-95"
